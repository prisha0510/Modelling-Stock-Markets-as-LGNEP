...
risk = []
for i in range(n-1):
    risk.append(float(sys.argv[4+i]))
...
def check_validity(current_strategy):
    ...
    if(bid_present==False or ask_present==False):
        return False
    if(min_difference>spread or min_difference<0):
        return False
    return True
...
def generate_lstm(iter_no):
    pred_lstm = np.load('lstm_predictions.npy')
    for l in range(options):
        strategy[1][l].price = pred_lstm[iter_no]
        choice = random.randint(0, 1)
        if(choice == 0):
            strategy[1][l].action = "buy"
        else:
            strategy[1][l].action = "sell"
...
def run(iter_no):
    ...
    generate_lstm(iter_no)
    initialise(iter_no, mean_price)
    # for i in range(n):
    #     for j in range(options):
    #         print(strategy[i][j].price)
    #         print(strategy[i][j].action)
